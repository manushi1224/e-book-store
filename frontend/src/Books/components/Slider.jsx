...
export default function Slider(props) {
  ...
  useEffect(() => {
    const fetchwishlist = async () => {
      try {
        ...
        const response = await axios.get(
          `https://manushi1224.pythonanywhere.com/create_api/wishlistdetail/?user=${userProfile.id}`
        );
  ...
  const saveWishlist = async (bookId) => {
    ...
    try {
      const response = await axios.post(
        "https://manushi1224.pythonanywhere.com/create_api/wishlist/",
        wishlist
      );
  ...
  const deleteWishlist = async (bookId) => {
    ...
    try {
      const response = await axios.post(
        "https://manushi1224.pythonanywhere.com/create_api/deletewishlist/",
        wishlist
      );
              ...
              <Card.Body>
                ...
                <div className="sci ms-2">
                  <div className="scidetail ms-3">
                    <button
                      className="btn btn-light scibutton"
                    ...
                    {flag ? (
                      isInWishlist(book.id) ? (
                        <button
                          ...
                          onClick={() => deleteWishlist(book.id)}
                        >
                          Remove
                        </button>
                      ) : (
                        <button
                          className="btn btn-light scibutton mt-2"
                          onClick={() => saveWishlist(book.id)}
                        >
                          Wishlist
                        </button>
                      )
                    ) : (
                      <button className="btn btn-light scibutton" disabled>
                        Wishlist
                      </button>
                    )}
                  </div>
